...
mem_init_mp(void)
{
	...
	// TODO:
	// Lab6: Your code here:
	int kern_stack_top = KSTACKTOP;
	int i;
	for (i = 0; i < NCPU; i++){
		boot_map_region(kern_pgdir, kern_stack_top - KSTKSIZE, KSTKSIZE, PADDR(percpu_kstacks[i]), PTE_W | PTE_P);
		kern_stack_top -= KSTKSIZE;
		kern_stack_top -= KSTKGAP;
	}
}
...
setupkvm()
{
	...
	if(pp){
		...
		/*
		pde = page2kva(pp);
    		boot_map_region(pde, UPAGES, ROUNDUP((sizeof(struct PageInfo) * npages), PGSIZE), PADDR(pages), PTE_U);
		boot_map_region(pde, KSTACKTOP - KSTKSIZE, KSTKSIZE, PADDR(bootstack), PTE_W);
    		boot_map_region(pde, IOPHYSMEM, ROUNDUP((EXTPHYSMEM - IOPHYSMEM), PGSIZE), IOPHYSMEM, PTE_W);
    		boot_map_region(pde, KERNBASE, -KERNBASE, 0, PTE_W);
		*/
	}
	return pgdir;
}
