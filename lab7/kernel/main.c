...
boot_aps(void)
{
	...
	void *mpentry_kaddr = KADDR(MPENTRY_PADDR);
	
	memmove(mpentry_kaddr, mpentry_start, (mpentry_end - mpentry_start));
	for (i = 0; i< ncpu; i++){
		if(cpus[i].cpu_id == cpunum()) continue;
		mpentry_kstack = percpu_kstacks[i] + KSTKSIZE;
		lapic_startap(cpus[i].cpu_id, PADDR(mpentry_kaddr));
