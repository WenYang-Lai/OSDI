...
{	
	if(argc < 2)
		cprintf("Need 1 hex number argument\n");
	else if (argv[1][0] >= 'a' && argv[1][0] < 'g')
		settextcolor(argv[1][0] - 'a', 0);
...
int mon_kerninfo(int argc, char **argv)
{
   ...
   */
	...
	cprintf("Kernel Code base=0x%x Kernel code size=%d\n", (int)&kernel_load_addr, (int)&etext - (int)&kernel_load_addr);
	cprintf("Kernel data base=0x%x Kernel data size=%d\n", (int)&data, (int)&end - (int)&data);
